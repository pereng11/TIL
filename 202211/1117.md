...
## 정적 경로에서 assets를 가져올 때 PUBLIC_URL 환경변수를 사용하는 이유
...
```
	...
	- 일반적으로 개발 단계에서 사용하는 정적파일 폴더 구조와 빌드 이후의 정적파일 폴더구조가 달라지기 때문에
- 정적 파일을 불러올 때의 경로 기준
	- 기본적으로 index.html 파일의 위치가 루트가 된다.
	- webpack-dev-server를 사용하며 config에 static 속성을 사용할 경우, 해당 값이 루트가 된다.
- 